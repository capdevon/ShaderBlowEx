...
/**
 * This bloom filter uses mipmaps of the main scene to generate a large bloom
 * effect. The final result is a scene texture with blur of the bright parts,
 * which is calculated by summing over the mipmap levels using the intensity
 * equation:
 * <code>final_color=bloomFactor*sum(mipmap_color*bloomPower^level)</code>
 * <p>
 * Adjustment of the bloom intensity and the downsampling coefficient shall be
...
public class MipmapBloomFilter extends Filter {
    ...
    public enum GlowMode {
        ...
        Objects,
        /**
         * Apply bloom to both bright parts of the scene and objects with glow
         * map.
         */
        SceneAndObjects
    }
    ...
    public enum Quality {
        /**
         * Uses an additional gaussian blur to smooth each mipmap level.
         * (Default)
         */
        High,
        /**
    ...
    @Override
    protected void initFilter(final AssetManager manager, RenderManager renderManager, ViewPort vp, int w, int h) {
        ...
        for (int ii = 0; ii < numPasses; ii++) {
            final int passWidth = Math.max(1, (int) (initialWidth / FastMath.pow(downSamplingCoef, (ii + 1))));
            final int passHeight = Math.max(1, (int) (initialHeight / FastMath.pow(downSamplingCoef, (ii + 1))));

            final Material passMat = new Material(manager, "ShaderBlowEx/MatDefs/MipmapBloom/MipmapSampler.j3md");
            final int jj = ii;
            mmPasses[jj] = new Pass() {
                ...
                public void beforeRender() {
                    if (jj == 0) {
                        ...
                        passMat.setTexture("Texture", mmPasses[jj - 1].getRenderedTexture());
                    }
            ...
            mmPasses[jj].init(renderManager.getRenderer(), passWidth, passHeight, texFormat, Format.Depth, 1, passMat);
            mmPasses[jj].getRenderedTexture().setMagFilter(Texture.MagFilter.Bilinear);
            mmPasses[jj].getRenderedTexture().setMinFilter(Texture.MinFilter.Trilinear);
    ...
    /**
     * Informs the FilterPostProcessor that this Filter needs the original scene
     * as texture.
     *
     ...
     * @return
     */
    protected Pass makeExtractPass(AssetManager manager, RenderManager renderManager, int w, int h) {
        ...
        extractPass.init(renderManager.getRenderer(), w, h, texFormat, Format.Depth, 1, extractMat);
    ...
    /**
     * Ad an additional Gaussian blur (one horizontal and one vertical pass) to
    ...
    private Texture2D gaussianBlur(AssetManager manager, final Texture2D texture) {
        ...
        // -----------------------------------------------------------------------------   
        final Material hBlurMat = new Material(manager, "ShaderBlowEx/MatDefs/MipmapBloom/HGaussianBlur.j3md");
        ...
        hBlur.init(renderManager.getRenderer(), w, h, texFormat, Format.Depth, 1, hBlurMat);
        ...
        // -----------------------------------------------------------------------------
        final Material vBlurMat = new Material(manager, "ShaderBlowEx/MatDefs/MipmapBloom/VGaussianBlur.j3md");
        ...
        vBlur.init(renderManager.getRenderer(), w, h, texFormat, Format.Depth, 1, vBlurMat);
    ...
    /**
     * Extracts the material's Glow techniques (if specified) and renders it
     * into the preGlowPass, if GlowMode is not Scene.
     *
    ...
    /**
     * Define the color threshold on which the bloom will be applied (0.0 to
     * 1.0)
     *
    ...
    /**
     * Setthe glow mode of the bloom filter.
     *
     ...
     */
    public float getDownSamplingCoef() {
        return downSamplingCoef;
    }

    /**
     * Sets the downsampling coefficient. Each mipmap (starting with level 0)
     * will have a reduced size defined by:
     * <code>mipmap_size=framebuffer_size/pow(downSamplingCoef^(level+1))</code>.
     * <p>
     * A lower value means less blur at higher computational cost, as the last
     * level will have a high resolution. The coefficient shall therefore be
     * chosen in such a way, that the highest (7th) level will have a resolution
