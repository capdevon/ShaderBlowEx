...
public class BetterGroundFogFilter extends Filter {
     ...
     *
     * @param direction normalized direction pointing from the sun, not towards
     * the sun
     */
    ...
    /**
     * set the fogs color when looking right into the sun through the fog,
     * default is 1.0, 0.9, 0.7 (slightly yellowish)
     *
    ...
    /**
     * used to adjust the amount of fog thats influenced by the sun, higher
     * values give smaller areas ( pow(sunFactor, sunShininess)), default is 8.0
     *
    ...
    /**
     * sets the height of the groundLevel (can be used to prevent from
     * underwaterfog). each fragment that is calculated to be below this
     * position is treated as if it was at this height, default is 0. set to
     * Float.MIN_VALUE to turn off
     *
    ...
    /**
     * sets the fogs boundary, x,y and z components should be its normal
     * pointing away from the fog, w is the distance, default is 0,1,0,-50 (that
     * means fog lays horizontally and starts at y=50 and). After the fogs plane
     * was updated, updateCameraPosition(pos) should be called
     *
    ...
    /**
     * should be called whenever the camera position changes, updates c-dot-f
     * and k so it does not have to be calculated for each fragment
     *
