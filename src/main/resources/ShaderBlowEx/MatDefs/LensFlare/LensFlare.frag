...
vec4 chromaticDistortion( in sampler2D tex, in vec2 sampleVec) {
    float r = texture2D(tex, sampleVec - distortion).r * intensity;
    float g = texture2D(tex, sampleVec).g * intensity;
    float b = texture2D(tex, sampleVec + distortion).b * intensity;
